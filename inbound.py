...
async def tts_with_retry(session: AgentSession, instructions: str, ctx: JobContext, agent: 'LogisticsAgent',
                         max_retries=10, initial_delay=0.5, max_delay=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.status_code == 429:
                logger.warning(f"TTS 429 error, attempt {attempt + 1}/{max_retries}, retrying in {retry_delay}s: {e}")
                await asyncio.sleep(retry_delay)
                retry_delay = min(retry_delay * 2, max_delay)
                continue
            else:
                raise
    logger.error(f"TTS failed after {max_retries} retries: 429 rate limit exceeded")
    fallback_message = (
        "I'm sorry, I'm having trouble with my voice system right now. "
        "I can still assist you with your shipping needs via text or try again shortly. "
        "Would you like to create a new order, get a quote, or ask a question?"
    )
    await log_transcript("assistant", fallback_message)
    await agent.send_slack_message(ctx,
                                   message=f"TTS rate limit exceeded after {max_retries} retries: {fallback_message}")
    return False
...
def validate_postal_code(postal_code: str, country: str = "US") -> bool:
    if country == "US":
        ...
        return bool(re.match(r"^[A-Za-z]\d[A-Za-z][ -]?\d[A-Za-z]\d$", postal_code.replace(" ", "")))
    return False

def get_distance_miles(origin_zip: str, dest_zip: str, origin_country: str = "US", dest_country: str = "US") -> float:
    url = "https://maps.googleapis.com/maps/api/distancematrix/json"
    origin = f"{origin_zip}, {origin_country}"
    dest = f"{dest_zip}, {dest_country}"
    params = {
        "origins": origin,
        "destinations": dest,
        "units": "imperial",
    ...
    try:
        ...
        raise ValueError(f"Failed to get distance: {e}")
...
def parse_user_query(query: str) -> List[str]:
    ...
    field_keywords = {
        ...
        "origin_zip": ["origin", "from", "starting", "pickup"],
...
class LogisticsAgent(Agent):
    def __init__(self, room: rtc.Room):
        super().__init__(
            instructions=(
                ...
                "Do not repeat the shipment ID in follow-up questions. Say 'Uhh... let me quickly check that shipment ID for you, okay? Just a sec...' when checking details. "
                "Fact-check shipment details using common sense (e.g., reject shipments to invalid locations like Antarctica). "
                "If the user is wasting your time, be patient 1 or 2 times, then end the call gracefully with the 'end_call' tool. "
                "Use the 'get_quote' tool when the user provides a valid shipment ID (e.g., SHP12345) or complete origin/destination ZIP codes (e.g., 10001 to L9T5A6). "
                "For follow-up questions about a quote, use the latest quote from context and respond only with the specific details requested (e.g., weight, stackability) without repeating the full quote. "
                "Use the 'calculate_ltl_quote' tool for precise LTL rates when the user provides ZIP codes, weight, freight class, and skid details. "
                "If the order has more than 7 skids, escalate to the rates team by collecting the customer's email and using the 'send_email' tool to notify the rates team. "
                "Ask for skid dimensions (length x width x height in inches), stackability (stackable or non-stackable), delivery appointment requirements, and urgency (e.g., same-day pickup). "
                "Use the 'create_new_order' tool to create a new shipping order when the user wants to send a load. Ask for origin ZIP code, destination ZIP code, weight (in pounds), freight class, skid details, stackability, and delivery requirements. "
                ...
                "Confirm details with phrases like 'Let me make sure I have that right' and end calls politely with 'You take care.'"
            )
        )
        self.call_context = {
            'call_type': 'waiting',
    ...
    async def _process_slack_message(self, response: str):
        ...
        await tts_with_retry(session, response, ctx=get_job_context(), agent=self)
    ...
    async def get_quote(
        ...
        try:
            ...
            if quote_info["quote_amount"] is not None:
                ...
                await self.send_slack_message(ctx, message=slack_message)
                await log_transcript("assistant", response)
                # Remove duplicate TTS call - let the LLM handle the response naturally
    ...
    async def update_order_tool(
        ...
        try:
            ...
            await log_transcript("assistant", response)
            # REMOVED: await tts_with_retry(session, response, ctx, self)
            return result
        except Exception as e:
    ...
    async def transfer_call(self, ctx: agents.RunContext, transfer_to: Optional[str] = None) -> dict:
        transfer_to = transfer_to or os.getenv("TRANSFER_PHONE_NUMBER", "+14807172012")
        if not transfer_to:
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        welcome_message = (
            f"Thank you for calling Polaris. How may I assist you?"
        )
